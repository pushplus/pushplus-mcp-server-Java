...
public class SendDto implements Serializable {
    ...
    @ToolParam(description = "发送渠道，默认wechat。枚举：wechat(微信公众号),webhook(第三方webhook),cp(企业微信应用),mail(邮箱),sms(短信),voice(语音),extension(插件),app(App),clawbot(微信ClawBot)。batchSend时多个渠道用逗号隔开，最多5个", required = false)
    private String channel;

    @ToolParam(description = "webhook编码（非URL），channel为webhook/cp/mail等时使用", required = false)
    private String webhook;

    @ToolParam(description = "渠道配置参数(原webhook参数)，与webhook等价；batchSend时多个用逗号隔开，与channel一一对应。如：\",config1,\"", required = false)
    ...
    @ToolParam(description = "好友令牌；微信公众号渠道填好友令牌，企业微信渠道填企业微信用户id；多人用逗号隔开；与topic互斥", required = false)
    private String to;
